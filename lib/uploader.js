...
export class Uploader {
	...
	enqueue(file_event) {
		...
		const already_queued = this.queue.some(e => {
			return e.directory === file_event.directory && e.relative_path === file_event.relative_path;
		});
	...
	async uploadFile(file_event) {
		const { host, username, port, remotePath } = this.profile;
		...
		const remote_path = posix.join(remotePath, normalized_relative_path);
		...
		logger.upload(file_event.display_path || file_event.relative_path, host, remote_path);
	}
	...
	async sync(local_root) {
		...
		logger.info(`Syncing ${local_root} → ${host}:${remotePath}...`);
		...
		await new Promise((resolve, reject) => {
			...
			child.on(`close`, (code) => {
				if(code === 0) {
					logger.success(`Sync complete → ${host}:${remotePath}`);
					resolve();
				}
				else {
