...
export class Uploader {
	...
	async uploadFile(file_event) {
		...
		const remote_path = posix.join(this.profile.remotePath, file_event.relative_path.replace(/\\/g, `/`));
		...
		logger.upload(file_event.relative_path, host);
	}

	async sync(local_root) {
		const { host, username, remotePath, port } = this.profile;
		const ssh_opts = buildSshOpts(this.profile);
