...
function normalizeRemotePaths(name, remote_path) {
	if(typeof remote_path === `string`) {
		...
		if(!trimmed_path) {
			throw new Error(`Profile "${name}" has an empty "remotePath" value.`);
		}
	...
	if(Array.isArray(remote_path)) {
		...
		const normalized_paths = remote_path.map((path, index) => {
			if(typeof path !== `string`) {
				throw new Error(`Profile "${name}" has a non-string "remotePath" entry at index ${index}.`);
			}

			const trimmed_path = path.trim();

			if(!trimmed_path) {
				throw new Error(`Profile "${name}" has an empty "remotePath" entry at index ${index}.`);
			}
...
export function loadConfig() {
	...
	for(const [name, profile] of Object.entries(config.profiles)) {
		...
		profile.remotePaths = normalizeRemotePaths(name, profile.remotePath);
